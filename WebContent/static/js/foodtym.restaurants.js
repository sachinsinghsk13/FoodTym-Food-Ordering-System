...
function load_recently_add_restaurnts() {
    $.ajax({
        url: '/FoodTymAdmin/Admin/Search?type=restaurant&by=recentlyadded',
        method: 'GET',
        success: (result) => {
            $('#result-item-container').children().remove().end().append(result);
            $('.result-item').on('click', (evt) => {
                var id = $(evt.currentTarget).attr('data-id');
                window.location = '/FoodTymAdmin/Admin/Restaurants?restaurantId=' + id;
...
function initialize_page() {
    ...
    $('#restaurant-search-bar').on('input', (evt) => {
        ...
        $.ajax({
            url: '/FoodTymAdmin/Admin/Search?type=restaurant&by=nameid&nameid=' + text,
            method: 'GET',
            success: (result) => {
                $('#result-item-container').children().remove().end().append(result);
                $('.result-item').on('click', (evt) => {
